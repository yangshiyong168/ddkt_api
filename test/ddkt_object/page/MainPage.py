...
class MainPage(AndroidClient):
    ...
    def getToastText(self) -> str:
        """获取toast"""
        toast = (By.XPATH, "//*[@class='android.widget.Toast']")
        get_text=self.getText(toast)
        print(get_text)
        return get_text
    ...
    def getImage(function):
        """封装一个截图的静态方法，执行测试用例时调用"""
        def get_ErrImage(self,*args, **kwargs):
            try:
                result = function(self,*args, **kwargs)
            except Exception as msg:
                file_name = os.getcwd() + "\\report\\" + time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()) +function.__name__+ '.png'
                self.driver.getScreenshotAsFile(file_name)
                allure.attach.file(file_name, function.__name__+"失败截图", attachment_type=allure.attachment_type.JPG)
                raise
            return result
        return get_ErrImage
